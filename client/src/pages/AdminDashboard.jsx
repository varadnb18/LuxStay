...
export default function AdminDashboard() {
  ...
  const createMutation = useMutation({
    ...
    onError: (e) => showError(e?.response?.data?.message || 'Failed to create hotel'),
  })

  const onChange = (e) => setForm(prev => ({ ...prev, [e.target.name]: e.target.value }))
  const addAmenity = () => setForm(prev => ({ ...prev, amenities: [...prev.amenities, ''] }))
  const onAmenityChange = (index, value) => setForm(prev => ({
    ...prev,
    amenities: prev.amenities.map((a, i) => (i === index ? value : a))
  }))
  const removeAmenity = (index) => setForm(prev => ({
    ...prev,
    ...
    <div className="max-w-6xl mx-auto px-6 py-10 grid gap-6 lg:grid-cols-3">
      <Card className="lg:col-span-1">
        ...
        <CardContent className="space-y-3">
          ...
          <Input name="pricePerNight" placeholder="Price per Night" value={form.pricePerNight} onChange={onChange} />
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <Button type="button" size="sm" variant="outline" onClick={addAmenity}>+</Button>
            </div>
            <div className="space-y-2">
              {form.amenities.map((a, idx) => (
                <div key={idx} className="flex items-center gap-2">
                  ...
                  />
                  <Button type="button" size="icon" variant="ghost" onClick={() => removeAmenity(idx)}>✕</Button>
                </div>
              ))}
            </div>
          </div>
          <Input name="images" placeholder="Image URLs (comma separated)" value={form.images} onChange={onChange} />
          <Button variant="gradient" onClick={() => createMutation.mutate()} disabled={createMutation.isPending}>
      ...
      <div className="lg:col-span-2">
        ...
        <div className="space-y-3">
          {hotels.map(h => (
            <div key={h._id} className="glass-effect rounded-lg p-4 flex items-center justify-between">
              <div>
                <div className="font-medium">{h.name}</div>
                <div className="text-sm text-muted-foreground">{h.location?.city}, {h.location?.country}</div>
              </div>
              <div className="text-primary font-semibold">${h.pricePerNight}</div>
            </div>
          ))}
        </div>
      </div>
